# Ingest — build a project catalog for a scope
...
belong to ONE scope. Your job: distill each into a compact catalog entry so grandma
knows the user's projects in this scope. The current scope memory, if any,
is in your system prompt.

## Steps
1. Read each `CLAUDE.md` at the given paths (they live in an `--add-dir`'d folder, so
   you can read them).
2. For each, judge: is this genuinely one of the user's work projects for this scope, or
...
3. For each kept project, write a compact entry into `<Scope>/projects.md`:
   ```
   ## <project-name>
   ...
   ```
   Also include any extra per-project fields the loaded scope memory asks for (a scope may
...
5. If the scope is brand new (no `facts.md`), create a minimal `<Scope>/facts.md`
   describing the engagement at a high level, and add the scope to `INDEX.md`.
6. Ensure `INDEX.md` has a pointer to `<Scope>/projects.md`.
...
- One scope only. Route nothing to other scopes.
- `projects.md` frontmatter: `scope`, `type: catalog`, `updated`.
